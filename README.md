...
- Публикация температуры оукружающей среды в топик `embedded/sensors/temperature` каждые 5 секунд
- Публикация ошибок в топик `embedded/errors`
- Подписка на команды в топике `embedded/control`
- Поддержка команд:
  - `restart` - перезапуск контроллера
  - `set_rgb` - установка значений в RGB светодиод
